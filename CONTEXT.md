...
**API Gateway**:
The public endpoint that receives external HTTPS traffic and forwards it into the local stack.
_Avoid_: Edge Gateway, Reverse Proxy

**Reverse Proxy & Load Balancer**:
The internal gateway layer that distributes requests to Backend API instances.
_Avoid_: API Gateway, Edge Proxy

**Backend API**:
The core application service that handles payment operations and can run multiple instances.
_Avoid_: Upstream

## Relationships

- The **API Gateway** forwards requests to the **Reverse Proxy & Load Balancer**
- The **Reverse Proxy & Load Balancer** routes traffic to one or more **Backend API** instances

## Example dialogue

> **Dev:** "When traffic hits the **API Gateway**, does it reach the **Backend API** directly?"
> **Domain expert:** "No — it goes through the **Reverse Proxy & Load Balancer**, which distributes to **Backend API** instances."

## Flagged ambiguities

- "Edge Gateway" was used to mean **API Gateway** — resolved: **API Gateway** is the public ngrok endpoint.
- "Upstream" was used to mean **Backend API** — resolved: use **Backend API** only.
