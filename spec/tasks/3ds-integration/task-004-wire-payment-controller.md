...
- **Local dependencies:**
  ...
  - `PaymentResponseDTO` — already exists with Lombok `@Builder`; `challengeId` and `acsUrl` fields added in task-005
...
3. Replace the existing `ResponseEntity<PaymentResponseDTO> response = duplicatePaymentHandler.buildNewPaymentResponse(saved);` block with conditional logic:
   - If `saved.getStatus() == PaymentStatus.CHALLENGE_PENDING`: build a 202 response using `PaymentResponseDTO.builder()` with `transactionId`, `status`, `challengeId` (from `saved.getChallengeId()` — may be null until task-005), `acsUrl` (from `saved.getChallengeAcsUrl()` — may be null until task-005)
...
- The `challengeId` and `acsUrl` fields on `PaymentResponseDTO` do not exist yet (added in task-005); use `saved.getChallengeId()` and `saved.getChallengeAcsUrl()` as placeholders in the builder, knowing they will be null until task-005 completes
