...
links:
  ...
  - spec/tasks/3ds-integration/task-006-callback-handler.md
---
...
- **Local dependencies:**
  - `ThreeDsCompletedEvent` record (from task-006): `public record ThreeDsCompletedEvent(UUID txId, boolean approved) {}`
  - `PaymentOrchestrationService.resumePaymentAfterAuth(UUID txId)` (from task-006)
  - `AuditLogService` — already exists
  - `ApplicationEventPublisher` / Spring Event infrastructure — already in use from task-006
